if [[ $# -gt 0 ]]
then
    echo Finding direct right recursive symbols in grammars... 1>&2
    trparse -l -t ANTLRv4 $@ 2> /dev/null | \
        trxgrep ' //parserRuleSpec[RULE_REF/text() = ruleBlock/ruleAltList/labeledAlt/alternative/*[name()="element"][last()]/atom/ruleref/*[1]/text()]' | trcaret
else
    echo Finding direct right recursive symbols in grammars... 1>&2
